...
def test_error_code_0():
    ...
    assert default_error.error_args == []
...
def test_error_code_10():
    default_timestamp = datetime.datetime(2018, 1, 2, 3, 4, 5, 6)
    default_context = "test"
    default_error = Error(ErrorCode.INVALID_SYNTAX, default_context, default_timestamp, ["toto"])
    assert default_error.error_type == ErrorCode.INVALID_SYNTAX
    assert default_error.error_code == 10
    assert default_error.error_message == "Invalid Syntax. Please use this syntax '%s'"
    assert default_error.context == default_context
    assert default_error.timestamp == default_timestamp
    assert default_error.error_args == ["toto"]
